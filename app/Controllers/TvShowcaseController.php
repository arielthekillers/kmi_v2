...
class TvShowcaseController extends Controller {
    ...
    public function uploadAudio() {
        ...
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            add_flash('Method tidak valid.', 'error');
            redirect('/teachers');
        }
        ...
        if (!isset($_FILES['audio']) || $_FILES['audio']['error'] !== UPLOAD_ERR_OK) {
            add_flash('Tidak ada file yang diunggah atau terjadi error.', 'error');
            redirect('/teachers');
        }
        ...
        if (!in_array($mime, $allowedMimes)) {
            add_flash('Format file tidak didukung. Gunakan MP3, OGG, WAV, atau M4A.', 'error');
            redirect('/teachers');
        }

        // Max 20 MB
        if ($file['size'] > 20 * 1024 * 1024) {
            add_flash('Ukuran file terlalu besar. Maksimal 20MB.', 'error');
            redirect('/teachers');
        }

        $targetDir  = __DIR__ . '/../../public/sound/';
        if (!is_dir($targetDir)) {
            mkdir($targetDir, 0755, true);
        }

        $targetPath = $targetDir . 'bgm.mp3';

        if (!move_uploaded_file($file['tmp_name'], $targetPath)) {
            add_flash('Gagal menyimpan file audio.', 'error');
            redirect('/teachers');
        }
