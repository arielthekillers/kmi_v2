<?php renderHeader("Petugas Absensi"); 
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
                Kelola sesi semester aktif dan penugasan bagian PBM (Proses Belajar Mengajar) untuk Tahun Ajaran <?= htmlspecialchars($activeAY['name']) ?>.
            </p>
        </div>
        ...
        <div class="flex">
            ...
            <div class="ml-3">
                <p class="text-sm text-blue-700">
                    Sistem hanya mengizinkan <strong>satu sesi semester aktif</strong> dalam satu waktu. Sesi yang <strong>"Terbuka"</strong> memungkinkan bagian PBM yang ditugaskan untuk menginput dan merekap ketidakhadiran santri.
                </p>
            </div>
        </div>
    </div>
        ...
        ?>
            <div class="bg-white rounded-xl shadow-sm border <?= $isActive ? 'border-indigo-500 ring-1 ring-indigo-500' : 'border-gray-200' ?> overflow-hidden flex flex-col transition-all">
                            ...
                            <div class="flex flex-col">
                                <div class="flex items-center gap-2">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold bg-indigo-100 text-indigo-800 tracking-tight">
                                        SMT <?= htmlspecialchars($session['semester']) ?>
                                    </span>
                                    <h3 class="text-base font-extrabold text-gray-900">Semester <?= htmlspecialchars($session['semester']) ?></h3>
                                </div>
                                <p class="text-[11px] text-gray-400 font-medium tracking-wide">Absensi Santri Smt <?= htmlspecialchars($session['semester']) ?></p>
                            </div>
                        </div>

                        <div class="flex items-center gap-5">
                            <div class="flex items-center gap-2 <?= !$isActive ? 'opacity-30 pointer-events-none' : '' ?>" title="<?= !$isActive ? 'Hanya bisa dibuka jika sesi AKTIF' : 'Buka/Tutup input absensi' ?>">
                                <span class="text-[11px] font-bold text-gray-400 uppercase tracking-widest">Input:</span>
                                <?php if ($isAdmin): ?>
                                    <form action="<?= url('/student-attendance/pbm/session/status') ?>" method="POST" id="form-toggle-<?= $session['id'] ?>" class="flex items-center">
                                        <?= csrf_token_field() ?>
                            ...
                            <?php elseif ($isAdmin): ?>
                                <form action="<?= url('/student-attendance/pbm/session/status') ?>" method="POST">
                                    <?= csrf_token_field() ?>
                                    <input type="hidden" name="id" value="<?= $session['id'] ?>">
                ...
                <div class="p-4 flex-1 flex flex-col min-h-0">
                    <form action="<?= url('/student-attendance/pbm/committee/update') ?>" method="POST" class="flex-1 flex flex-col min-h-0">
                        <?= csrf_token_field() ?>
                        ...
                        <div class="flex-1 overflow-y-auto border border-gray-200 rounded-lg bg-white mb-3 shadow-inner" style="max-height: 200px;">
                            <table class="min-w-full divide-y divide-gray-100">
                                <thead class="bg-gray-50/80 sticky top-0 z-10 backdrop-blur-sm">
                                    <tr>
                                        <th class="px-3 py-2 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">Nama Personil PBM</th>
                                        <?php if ($isAdmin): ?>
                                    ...
                                    <?php if ($count === 0): ?>
                                        <tr class="empty-state">
                                            <td colspan="2" class="px-3 py-4 text-center text-[10px] text-gray-400 italic">
                                                Belum ada bagian PBM ditugaskan
                                            </td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
