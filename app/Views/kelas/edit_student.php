<main class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6 flex items-center gap-4">
        <a href="<?= url('/classes/detail?id=' . $kelas_id . '&tab=santri') ?>" class="p-2 bg-white border border-gray-200 rounded-lg text-gray-500 hover:text-indigo-600 transition-colors">
    ...
    <form action="<?= url('/kelas/update-student') ?>" method="POST" id="studentForm" class="space-y-8">
        ...
        <div class="bg-white shadow-sm border border-gray-200 rounded-xl p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="text" name="nis" value="<?= $student['nis'] ?? '' ?>" required <?= auth_get_role() === 'admin' ? '' : 'readonly' ?> class="mt-1 block w-full rounded-lg <?= auth_get_role() === 'admin' ? 'border-gray-300' : 'bg-gray-100 border-gray-200' ?> shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm p-2.5 border">
                    <?php if (auth_get_role() !== 'admin'): ?>
                        <p class="text-[10px] text-gray-400 mt-1">NIS hanya dapat diubah oleh Admin.</p>
                    <?php endif; ?>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">NISN</label>
                    <input type="text" name="nisn" value="<?= $student['nisn'] ?? '' ?>" <?= auth_get_role() === 'admin' ? '' : 'readonly' ?> class="mt-1 block w-full rounded-lg <?= auth_get_role() === 'admin' ? 'border-gray-300' : 'bg-gray-100 border-gray-200' ?> shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm p-2.5 border">
                    <?php if (auth_get_role() !== 'admin'): ?>
                        <p class="text-[10px] text-gray-400 mt-1">NISN hanya dapat diubah oleh Admin.</p>
                    <?php endif; ?>
                </div>
                <div>
