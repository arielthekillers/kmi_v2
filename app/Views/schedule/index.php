...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
                ...
                <?php if ($selectedKelasId): ?>
                    <div class="flex items-center gap-2">
                        <button onclick="openEditModal('editJadwalModal')" class="bg-indigo-600 hover:bg-indigo-700 text-white px-3.5 py-2 rounded-lg text-sm font-medium shadow-sm flex items-center gap-2 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                            Edit Jadwal Utama
                        </button>
                        <button onclick="openEditModal('editPenggantiModal')" class="bg-amber-600 hover:bg-amber-700 text-white px-3.5 py-2 rounded-lg text-sm font-medium shadow-sm flex items-center gap-2 transition-colors">
                            <i class="ri-swap-line text-base"></i>
                            Pergantian Pengajar / Pelajaran
                        </button>
                    </div>
                <?php endif; ?>
            </div>
                            ...
                            <tr>
                                ...
                                <?php foreach ($hours as $h): ?>
                                    <td class="px-2 py-3 text-center align-top border-l border-gray-100 hover:bg-gray-50 transition-colors">
                                        ...
                                        if ($selectedKelasId) {
                                            // Class View
                                            $primary = $primaryJadwal[$day][$h] ?? [];
                                            $replacement = $replacementJadwal[$day][$h] ?? [];

                                            $hasReplacement = !empty($replacement['mapel']) && 
                                                              ($replacement['mapel'] != ($primary['mapel'] ?? '') || $replacement['pengajar'] != ($primary['pengajar'] ?? ''));

                                            if ($hasReplacement) {
                                                // 1. Render Old (Strikethrough / Coret)
                                                $oldMapelName = $pelajaranData[$primary['mapel']]['nama'] ?? '-';
                                                $oldPengajarName = $pengajarData[$primary['pengajar']]['nama'] ?? '-';
                                                echo '<div class="opacity-50 line-through decoration-red-500 decoration-2 mb-2 p-1 bg-gray-50 rounded border border-gray-200" title="Jadwal Sebelumnya">';
                                                echo '<div class="text-xs font-semibold text-gray-500 line-clamp-1">' . htmlspecialchars($oldMapelName) . '</div>';
                                                echo '<div class="text-[10px] text-gray-400 line-clamp-1">' . htmlspecialchars($oldPengajarName) . '</div>';
                                                echo '</div>';

                                                // 2. Render New Active (Replacement)
                                                $newMapelName = $pelajaranData[$replacement['mapel']]['nama'] ?? '-';
                                                $newPengajarName = $pengajarData[$replacement['pengajar']]['nama'] ?? '-';
                                                echo '<div class="font-semibold text-indigo-700 mb-1 line-clamp-2" title="' . htmlspecialchars($newMapelName) . '">' . htmlspecialchars($newMapelName) . '</div>';
                                                echo '<div class="text-xs text-gray-500 line-clamp-2" title="' . htmlspecialchars($newPengajarName) . '">' . htmlspecialchars($newPengajarName) . '</div>';
                                            } else {
                                                // Normal Active Slot
                                                $mapelId = $primary['mapel'] ?? '';
                                                $pengajarId = $primary['pengajar'] ?? '';
    ...
    <!-- 1. Edit Modal: Jadwal Utama (Hard Edit / Fix Typo) -->
    ...
    <div id="editJadwalModal" class="hidden fixed z-50 inset-0 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all w-full sm:my-8 sm:align-middle sm:max-w-6xl sm:w-full">
                <form action="<?= url('/schedule/store') ?>" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin memperbarui Jadwal Utama ini? (Perbaikan typo / koreksi tidak menambah riwayat)');">
                    <?= csrf_token_field() ?>
                    <input type="hidden" name="kelas_id" value="<?= htmlspecialchars($selectedKelasId) ?>">
                    <input type="hidden" name="mode" value="utama">
                    
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="flex items-center justify-between mb-4 border-b pb-3">
                            <div>
                                <h3 class="text-lg font-bold text-gray-900">Edit Jadwal Utama: Kelas <?= htmlspecialchars($k['tingkat']) ?>-<?= htmlspecialchars($k['abjad']) ?></h3>
                                <p class="text-xs text-gray-500">Perbaikan jadwal awal / koreksi typo. Perubahan di sini langsung memperbarui data jadwal utama tanpa membuat entri baru.</p>
                            </div>
                            <span class="px-2.5 py-1 text-xs font-semibold bg-indigo-100 text-indigo-700 rounded-md">Jadwal Utama</span>
                        </div>
    ...
    <!-- 2. Modal: Pergantian Pengajar / Pelajaran (Pergantian Resmi -> Tampil data ke-2) -->
    <div id="editPenggantiModal" class="hidden fixed z-50 inset-0 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onclick="toggleModal('editPenggantiModal')"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all w-full sm:my-8 sm:align-middle sm:max-w-6xl sm:w-full">
                <form action="<?= url('/schedule/store') ?>" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyimpan Pergantian Pengajar / Pelajaran ini? (Pelajaran/Pengajar baru akan tercatat di riwayat)');">
                    <?= csrf_token_field() ?>
                    <input type="hidden" name="kelas_id" value="<?= htmlspecialchars($selectedKelasId) ?>">
                    <input type="hidden" name="mode" value="pengganti">
                    
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="flex items-center justify-between mb-4 border-b pb-3">
                            <div>
                                <h3 class="text-lg font-bold text-amber-900">Pergantian Pengajar / Pelajaran: Kelas <?= htmlspecialchars($k['tingkat']) ?>-<?= htmlspecialchars($k['abjad']) ?></h3>
                                <p class="text-xs text-gray-500">Pilih mata pelajaran atau pengajar baru yang menggantikan di pertengahan tahun ajaran. Pelajaran/pengajar lama akan tetap tersimpan di riwayat.</p>
                            </div>
                            <span class="px-2.5 py-1 text-xs font-semibold bg-amber-100 text-amber-800 rounded-md">Jadwal Pengganti</span>
                        </div>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200 text-xs">
                                    <?php foreach ($days as $day): ?>
                                        <tr>
                                            ...
                                            <?php foreach ($hours as $h): 
                                                // Display the 2nd / replacement entry if available, fallback to empty
                                                $slot = $replacementJadwal[$day][$h] ?? [];
                                                $currMapel = $slot['mapel'] ?? '';
                                                $currPengajar = $slot['pengajar'] ?? '';
                                            ?>
                                                <td class="px-2 py-2 border-l border-gray-100 bg-white">
                                                    <div class="space-y-2">
                                                        <div class="border-l-4 border-amber-400 pl-2 rounded-r bg-amber-50/60 py-1">
                                                            <span class="text-[10px] uppercase font-bold text-amber-700 block mb-0.5 px-0.5">Mapel Pengganti</span>
                                                            <select name="schedule[<?= $day ?>][<?= $h ?>][mapel]" class="tom-select block w-full border-gray-300 rounded-md shadow-sm focus:ring-amber-500 focus:border-amber-500 text-xs py-1">
                                                                <option value="">- Tidak ada pengganti (Ikut Utama) -</option>
                                                                <?php foreach ($subjectOptions as $p): ?>
                                                                    <option value="<?= $p['id'] ?>" <?= $currMapel == $p['id'] ? 'selected' : '' ?>>
                                                                        <?= htmlspecialchars($p['nama']) ?>
                                                                    </option>
                                                        ...
                                                        <div class="border-l-4 border-orange-400 pl-2 rounded-r bg-orange-50 py-1">
                                                            <span class="text-[10px] uppercase font-bold text-orange-600 block mb-0.5 px-0.5">Pengajar Pengganti</span>
                                                            <select name="schedule[<?= $day ?>][<?= $h ?>][pengajar]" class="tom-select block w-full border-gray-300 rounded-md shadow-sm focus:ring-amber-500 focus:border-amber-500 text-xs py-1">
                                                                <option value="">- Pilih Pengajar -</option>
                                                                <?php foreach ($teacherOptions as $p): ?>
                                                                    <option value="<?= $p['id'] ?>" <?= $currPengajar == $p['id'] ? 'selected' : '' ?>>
                                                                        <?= htmlspecialchars($p['nama']) ?>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-gray-100">
                        ...
                        <button type="button" onclick="toggleModal('editPenggantiModal')" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                            Batal
                        </button>
                    </div>
                </form>
...
<script>
    ...
    function openEditModal(modalId) {
        toggleModal(modalId || 'editJadwalModal');
    }
</script>
