<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div class="bg-indigo-600 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
            ...
            <div class="relative z-10">
                <p class="text-indigo-100 text-sm font-bold uppercase tracking-wider mb-1">Total Santri Aktif</p>
                <h3 class="text-4xl font-black mb-2"><?= number_format($stats['total']) ?> <span class="text-lg font-medium opacity-75">Santri</span></h3>
                <p class="text-indigo-100 text-sm">Santri dengan status aktif saat ini.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm flex items-center gap-5">
            ...
            <div>
                <p class="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">Santri Putra (L)</p>
        ...
        <div class="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm flex items-center gap-5">
            ...
            <div>
                <p class="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">Santri Putri (P)</p>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-6">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
            <h3 class="text-gray-800 font-bold flex items-center gap-2">
                <i class="ri-bar-chart-grouped-line text-blue-500"></i> Rata-rata Kelengkapan Data per Kategori Kelas
            </h3>
        </div>
        <div class="p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    ...
                    <div class="p-4 border border-gray-100 rounded-xl bg-gray-50 hover:bg-white hover:shadow-sm transition-all">
                        <div class="flex justify-between items-center mb-3">
                            <h4 class="font-bold text-gray-800 text-sm truncate" title="Kelas <?= htmlspecialchars($tingkat) ?>"><?= $tingkat === 'Belum Ada Kelas' ? 'Belum Ada Kelas' : 'Kelas ' . htmlspecialchars($tingkat) ?></h4>
                            <span class="text-xs font-bold px-2 py-1 rounded bg-white border <?= $pct >= 75 ? 'border-emerald-200 text-emerald-600' : ($pct >= 50 ? 'border-yellow-200 text-yellow-600' : 'border-red-200 text-red-600') ?> shadow-sm"><?= $pct ?>%</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="<?= $pct >= 75 ? 'bg-emerald-500' : ($pct >= 50 ? 'bg-yellow-400' : 'bg-red-500') ?> h-2 rounded-full" style="width: <?= $pct ?>%"></div>
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                <h3 class="text-gray-800 font-bold flex items-center gap-2">
                    <i class="ri-file-list-3-line text-emerald-500"></i> Progres Keseluruhan Kelengkapan Data
                </h3>
            </div>
            <div class="p-6">
                <div class="space-y-5">
